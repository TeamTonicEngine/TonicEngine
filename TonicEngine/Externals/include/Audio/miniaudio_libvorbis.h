...
#if !defined(MA_NO_LIBVORBIS)
	{
		...
		while (totalFramesRead < frameCount) {
			...
			if (format == ma_format_f32) {
				...
				libvorbisResult = ov_read(&pVorbis->vf, reinterpret_cast<char*>(ma_offset_pcm_frames_ptr(pFramesOut, totalFramesRead, format, channels)), framesToRead * ma_get_bytes_per_frame(format, channels), 0, 2, 1, NULL);
				if (libvorbisResult < 0) {
					result = MA_ERROR;  /* Error while decoding. */
					break;
				}
