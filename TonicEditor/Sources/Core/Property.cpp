...
void Core::Applications::EditorWindow::DrawProperty(u64 _id)
{
	...
	//bool hasAudioMixer = ENGINE.ENT_MNGR->HasComponent<ECS::Components::AudioMixerComponent>(_id);
		...
		if (open)
		{
			...
			if (ImGui::BeginPopup("audioList", NULL))
			{
				for (const auto& r : ENGINE.RES_MNGR->audioList)
				{
					if (ImGui::MenuItem(r->path.string().c_str()))
					{
						//name = ENGINE.ENT_MNGR->GetComponent<ECS::Components::AudioSourceComponent>(_id).p_sound->path.string().c_str();
						ENGINE.ENT_MNGR->GetComponent<ECS::Components::AudioSourceComponent>(_id).p_sound = std::reinterpret_pointer_cast<Resources::Sound>(r);
					}
				}
				ImGui::EndPopup();
			}

			if (ENGINE.ENT_MNGR->GetComponent<ECS::Components::AudioSourceComponent>(_id).p_sound)
			{
				if (ImGui::Button("play"))
					ENGINE.ENT_MNGR->GetComponent<ECS::Components::AudioSourceComponent>(_id).p_sound->Play();

				ImGui::SameLine();

				if (ImGui::Button("pause"))
					ENGINE.ENT_MNGR->GetComponent<ECS::Components::AudioSourceComponent>(_id).p_sound->Pause();

				ImGui::SameLine();

				if (ImGui::Button("stop"))
					ENGINE.ENT_MNGR->GetComponent<ECS::Components::AudioSourceComponent>(_id).p_sound->Stop();
			}
	...
	//if (hasAudioMixer)
	//{
		//bool open = CreateNode<ECS::Components::AudioListenerComponent>(_id, "AudioMixer");
	//}
