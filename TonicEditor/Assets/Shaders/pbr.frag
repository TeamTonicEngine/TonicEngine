...
#define MAX_DIR_LIGHTS 24
#define MAX_POINT_LIGHTS 24
#define MAX_SPOT_LIGHTS 24

uniform uint nbDirLights;
uniform uint nbPointLights;
uniform uint nbSpotLights;
...
void main()
{		
    vec3 albedo     = pow(texture(material.diffuse, TexCoords).rgb, vec3(gammaCorrection));
    ...
    vec3 Lo = vec3(0.0);
        for(uint i = 0u; i < nbDirLights; ++i) 
    {
        // calculate per-light radiance
        vec3 L = dirLights[i].direction;
        vec3 H = normalize(V + L);
        vec3 radiance = dirLights[i].specular;
    ...
    for(uint i = 0u; i < nbSpotLights; ++i) 
    {
        ...
        // spotlight intensity
        float theta = dot(L, -spotLights[i].direction); 
        float coneAttenuation = smoothstep(spotLights[i].outerCutOff, spotLights[i].cutOff, theta);
        ...
        vec3 radiance = spotLights[i].specular * attenuation * coneAttenuation;
